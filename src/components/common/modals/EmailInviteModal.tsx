...
export default function AddPatientModal({ isOpen, onClose, onSubmit }: AddPatientModalProps) {
  ...
  return (
    <Overlay show={isOpen} onClose={onClose}>
      <BaseModal title="Agregar Paciente" onClose={onClose} speak={false} className='gap-10 p-12 rounded-3xl'>
        <form onSubmit={handleSubmit} className="p-6 space-y-6 flex flex-col">
          <div>
            ...
            <Input name={'childName'} onChange={handleChange} id='childName' value={patientData.childName} required={true} className='w-full' />
          </div>
          <div>
            <label htmlFor="guardianName" className="block text-sm font-medium text-gray-700">
              Nombre y apellido del tutor *
            </label>
            <Input name={'guardianName'} onChange={handleChange} id='guardianName' value={patientData.guardianName} required={true} className='w-full' />
          </div>
          <div>
            <label htmlFor="guardianEmail" className="block text-sm font-medium text-gray-700">
              Email del tutor *
            </label>
            <Input name={'guardianEmail'} type='email' onChange={handleChange} id='guardianEmail' value={patientData.guardianEmail} required={true} className='w-full' />
          </div>
          <div>
            <label htmlFor="childAge" className="block text-sm font-medium text-gray-700">
              Edad del niño/a *
            </label>
            <Input name={'childAge'} type='number' onChange={handleChange} id='childAge' value={patientData.childAge} required={true} className='w-full' />
          </div>
          <button
            type="submit"
